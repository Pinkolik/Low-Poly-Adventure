...
float Map::findIntersectionCoefficient(Node &anotherNode, glm::vec3 direction) {
	vector<float> coefficients;
	for (Node &node : nodes) {
		float coefficient = node.findIntersectionCoefficient(anotherNode,
				direction);
		if (coefficient == MAXFLOAT) {
			continue;
		}
...
Mesh Map::prcoessMesh(tinygltf::Model &gltfModel, tinygltf::Mesh &gltfMesh) {
	...
	for (size_t i = 0; i < gltfMesh.primitives.size(); i++) {
		...
		for (size_t j = 0; j < normals.size(); j++) {
			Vertex vertex;
			vertex.position = glm::vec3(positions[j][0], positions[j][1],
					positions[j][2]);
			vertex.normal = glm::vec3(normals[j][0], normals[j][1],
					normals[j][2]);
			vertex.texcoord = glm::vec2(texcoord[j][0], texcoord[j][1]);
			vertices.push_back(vertex);
		}
		vector<unsigned short> indices = getUnsignedShortVector(gltfModel,
...
vector<unsigned short> Map::getUnsignedShortVector(tinygltf::Model &gltfModel,
