...
void Player::processKeyboard(GLFWwindow *window, Map &map, float deltaTime) {
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
		direction += glm::normalize(front * glm::vec3(1.0f, 0, 1.0f))
				* velocity;
	}
	...
	float intersectionCoefficient = map.findIntersectionCoefficient(
			*getPlayerBox(), direction) - 0.001f;
	Node projection = *getPlayerBox();
	projection.setTranslation(
			projection.getTranslation() + direction * intersectionCoefficient);
...
void Player::tick(Map &map, float deltaTime) {
	...
	float intersectionCoefficient = map.findIntersectionCoefficient(
			*getPlayerBox(), down) - 0.001f;
	Node fallProjection = *getPlayerBox();
	fallProjection.setTranslation(
			fallProjection.getTranslation() + down * intersectionCoefficient);
